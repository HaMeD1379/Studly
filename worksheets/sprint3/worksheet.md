...
#### Team Member 4: Anthony

This was my first time working on the frontend, so my contribution to the group involved a lot of learning on the go, so I used AI namely ChatGPT a handful of  times to fix formatting issues, but the biggest area I would say AI was used in personally was for my testing. A suitable example would be when i was writing tests for our friends page component, and the friends page made multiple api calls, so it got a bit confusing trying to keep track with different response structures for each API call, chatGPT provided what it thought was a suitable implementation which didnt work, and after going back and forth with it multiple times, eventually i had to sit down and draw out the responses for each API call which helped me fix the issues with the test. I found that AI was good at generating test cases which helped for edge testing and boundary testing. However these generated test cases weren’t always copy and paste ready especially when I was testing loader functions, or components that made API calls probably because I didnt always provide it context in terms of what the api expected, I usually had to go in and rewrite/rename fields. Overall, if I had to pick out one thing I learnt from its responses, I think having to go back in and rewrite the test, made me more comfortable with mocking objects which was something I never really understood.

#### Team Member 5: Shiv

I used Github Copilot as an assistant throughout the project, mainly for writing tests, building Feed API, and Friends API. Copilot was helpful in framing the initial strucuture and suggesting query patterns. I made the descision of the routes myself with choosing separate queries. In my most recent Feed API, Copilot quickly generated the updated logic when I needed to reverse the sorting order based on the developer's feedback. After that I verified and tested myself to make sure that everything was working fine. The tool excelled at generating comphrehensive test scenerios which I was not able to think of which helped me achieve more than 95% of the line coverage in most API endpoints. This saved me significant time on edge case testing. I found it a valuable asset as it is a productivity aid for repitive tasks and boilerplate code and I focused on the problem solving validation aspects myself.
