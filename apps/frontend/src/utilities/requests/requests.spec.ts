...
import { failureMessageMock, failureRequestMock, failureStatusMock, successDataMock, successRequestMock } from '~/mocks';

describe('requests', () => {
  it('returns with data', async () => {
    vi.stubGlobal('fetch', vi.fn(() => successRequestMock));
  ...
  it('returns with error', async () => {
    vi.stubGlobal('fetch', vi.fn(() => failureRequestMock));
