...
import { searchFriends, sendFriendRequest } from '~/api';
import { FriendsActionResponse } from '~/types';

export const action = async ({ request }: ActionFunctionArgs): Promise<FriendsActionResponse> => {
  ...
  switch (formType) {
    case "searchFriend": {
      ...
      return {data: res.data?.data, formtype: "searchFriends"} as FriendsActionResponse;
    }
