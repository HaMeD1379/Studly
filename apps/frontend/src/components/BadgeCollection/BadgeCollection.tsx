import { Flex, SegmentedControl, Pagination, Text, SimpleGrid, Box } from '@mantine/core';
import { useEffect, useState } from 'react';
import { BadgeCollectionFilterStatus, BADGES_PER_COLLECTION_PAGE } from '~/constants';
...
export const BadgeCollection = ({ unlockedBadges, allBadges }: BadgeCollectionProps) => {
  const [page, setPage] = useState<number>(1);
  const [totalPages, setTotalPages] = useState<number>(1);
  const [filterStatus, setFilterStatus] = useState<string>(BadgeCollectionFilterStatus.Unlocked)
  const [visibleValues, setVisibleValues] = useState<VisibleBadgeValue[]>([]);

  useEffect(() => {
    const rowsUsed = filterStatus === BadgeCollectionFilterStatus.AllBadges ? allBadges : unlockedBadges;
    const rowsVisible = rowsUsed.slice((page - 1) * BADGES_PER_COLLECTION_PAGE, page * BADGES_PER_COLLECTION_PAGE);
  ...
  return (
    <Flex my='lg' gap='lg' direction='column'>
      ...
      {
        filterStatus === BadgeCollectionFilterStatus.Unlocked && unlockedBadges.length === 0 ? 
      (
        <Flex p={142} bdrs={8} bd='1px solid lightgray' direction='column' align='center'>
          <Text c='gray'>No badges unlocked yet!</Text>
          <Text c='gray'>Start studying to earn badges!</Text>
        </Flex>
      ) :
        filterStatus === BadgeCollectionFilterStatus.AllBadges && allBadges.length === 0 ?
      (
        <Flex p={142} bdrs={8} bd='1px solid lightgray' direction='column' align='center'>
              ...
              <Flex key={`badge-value-${index}`} p={24} gap={4} bd='1px solid lightgray' bdrs={8} direction='column'>
                ...
                {timeUnlocked ? (
                    <Flex align='center' gap={4}>
                      ...
                      <Text c='#04A740'>Unlocked {formatToYYYYMMDD(timeUnlocked)}</Text>
                    </Flex>
                  ) :
